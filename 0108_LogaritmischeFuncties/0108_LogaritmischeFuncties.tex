...
\begin{align*}
\log_a(x\cdot y) &= \log x + \log y && \mbox{logaritme van een product}\\
...
\pagebreak
\paragraph*{Veranderen van grondtal}
\begin{mdframed}
...
\begin{itemize}
  ...
  \begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
    ...
    \draw[color=gray] (0.6,4) node[anchor=north west] {$y=2^x$};
    \draw (3.5,1.8) node[anchor=north west] {$y=\log_2 x$};
  \end{tikzpicture}
  \end{center}
...
\begin{oefening}
Bepaal de waarde van de volgend logaritme.
\begin{exlist}{3}
  \item $\log 1000$
  \item $\log_2 32$
