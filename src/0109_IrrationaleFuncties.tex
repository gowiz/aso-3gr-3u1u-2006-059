...
Los grafisch $\sqrt{x+1}=3$ op.

We kunnen van het linker- en rechter lid van de vergelijking een functie maken. Als we deze twee functies dan in één zelfde assenstelsel tekenen, dan is de abscis (de $x$-waarde) van mogelijke snijpunten telkens een oplossing van de oorspronkelijke vergelijking.

We krijgen dus
$$\underbrace{\sqrt{x+1}}_f=\underbrace{3}_g$$

We tekenen dus twee functies:
$$f(x)=\sqrt{x+1}$$
en
$$g(x)=3$$
...
xtick={-2.0,0.0,...,12.0},
ytick={-2.0,0.0,...,4.0},]
\clip(-2.7,-2.7) rectangle (13.3,5.3);
\draw[line width=2.4pt,smooth,samples=1000,domain=-1:13.3] plot(\x,{sqrt((\x)+1)});
\draw[line width=2.4pt,smooth,samples=100,domain=-2.7:13.3] plot(\x,{3});
\draw (1.3,3.8) node[anchor=north west] {$g$};
\draw (2.7,1.9) node[anchor=north west] {$f$};
\draw [color=uuuuuu,->,line width=1.2pt] (8.,3.) -- (8.,0.);
\begin{scriptsize}
\draw [fill=uuuuuu] (8.,3.) circle (2.0pt);
\draw[color=uuuuuu] (7.607004921154812,3.467624944291244) node {$(8,3)$};
\end{scriptsize}
\end{axis}
\end{tikzpicture}
\end{center}
