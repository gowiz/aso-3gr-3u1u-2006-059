...
\begin{oefening}
We gaan naar de bakker en geven in totaal 12.40 euro uit voor chocoladekoeken en croissants. We weten dat een chocoladekoek 0.65 euro en een croissant 0.60 euro kost. Hoeveel chocoladekoeken hebben we mee?
\end{oefening}
...
We hebben dus al één oplossing gevonden, namelijk $x_0=0$. In de factor $-8x^2+18x+35$ kunnen ook nog oplossingen zitten. We vinden dat er twee oplossing zijn m.b.v. de discriminant $D=1444$. De oplossingen zijn $x_1=\frac{7}{2}$ en $x_2=-\frac{5}{4}$.
...
\item Dus $x=-1$ is een oplossing. Dit wil zeggen dat $x-(-1)$ of korter $x+1$ een deler is van de de veelterm. We herschrijven de vergelijking:
  \begin{eqnarray*}
    x^3-4x^2+x+6&=&0\\
    \lra (x+1)\underbrace{(\qquad ? \qquad)}_{\mbox{quotiënt}}&=&0\\
  \end{eqnarray*}
\item Het quotiënt vinden we m.b.v. het algoritme van Horner:
  \begin{center}
...
Als we een deler zoeken van de vorm $(x-a)$ dan maken we gebruik van de reststelling die we in het vierde middelbaar geleerd hebben. Deze stelt
\subsubsection*{Reststelling}
\begin{mdframed}
  De rest van de deling van een veelterm door $(x-a)$ vinden we als we op de plaats van $x$ de waarde $a$ invullen.
\end{mdframed}

We noemen een veelterm nu deelbaar door $(x-a)$ als deze rest nul is.
...
\begin{mdframed}
  ...
  \item Nulwaarden: $ax^2+bx+c=0$
    ...
    \begin{itemize}
    \item $D=b^2-4ac$
    \item $D>0$ dan twee verschillende nulwaarden $x_1=\dfrac{-b-\sqrt{D}}{2a}$ en $x_2=\dfrac{-b+\sqrt{D}}{2a}$
...
Onderstaand vooral oude examenvragen.
  ...
  \begin{center}
    \begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
      \begin{axis}[
        ...
        \clip(-3.4,-110.8) rectangle (17.1,163.9);
        \draw[line width=2.pt,smooth,samples=100,domain=-3.4:17.1] plot(\x,{100-6.25*((\x)-9)^(2)});
        \draw (14,100) node {\scriptsize $ P = 100 - 6.25(x-9)^2 $};
      \end{axis}
    \end{tikzpicture}
  \end{center}
  Een bedrijf maakt een bepaald type kinderspeelgoed. De jaarlijkse winst wordt gemodelleerd door
  $$ P = 100 - 6.25(x-9)^2 $$
  waarbij $P$ de winst is in duizend euro en $x$ de verkoopsprijs van het speelgoed in euro. Een sketch van de $P$ in functie van $x$ wordt gegeven in de grafiek hierboven.
  \begin{enumerate}[(a)]
  \item Verklaar waarom dat 15 euro geen goede verkoopsprijs is voor het speelgoed.
  \item Gegeven dat het bedrijf een jaarlijkse winst maakt van $80000$ euro, bepaal dan de laagst mogelijke verkoopsprijs van het speelgoed.
  \item Wat is de maximaal mogelijke jaarlijkse winst?
  \item Wat moet de verkoopsprijs van het speelgoed zijn bij de maximaal mogelijke jaarlijkse winst?
  \end{enumerate}
\end{oefening}

\begin{oefening}{\em Bron: Pearson Edexcel, Pure Mathematics, may 2018}
  $$g(x)=4x^3-12x^2-15x+50$$
  ...
  \begin{center}
    \begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
      \begin{axis}[
        ...
        \clip(-4.403628411996042,-84.61940293696044) rectangle (6.409714395319561,114.61940293696033);
        \draw[line width=2.pt,smooth,samples=100,domain=-4.403628411996042:6.409714395319561] plot(\x,{4*(\x)^(3)-12*(\x)^(2)-15*(\x)+50});
        \draw (5,50) node {\scriptsize $ y=g(x) $};
      \end{axis}
    \end{tikzpicture}
  \end{center}
  \begin{enumerate}[(a)]
  \item Gebruik de reststelling om aan te tonen dat $(x+2)$ een deler is van $g(x)$.
  \item Toon nu aan dat $g(x)$ geschreven kan worden als
    $$g(x)=(x+2)(ax+b)^2$$
    en waarbij $a$ en $b$ gehele getallen zijn.
  \item Gebruik je antwoord uit deel (b) en de bovenstaande schets om de waarden van $x$ af te leiden waarvoor:
    \begin{enumerate}[(i)]
    \item $g(x) \leq 0$
    \item $g(2x) = 0$
    \end{enumerate}
...
\begin{oefening}{\em Bron: Pearson Edexcel, Pure Mathematics, may 2018}
  \begin{enumerate}[(a)]
  \item Toon aan dat $x^2-8x+17 > 0$ voor alle reële waarden van $x$.
  \item {\em Als ik drie optel bij een getal en ik kwadrateer de som, dan is het resultaat groter dan de vierkantswortel van het originele getal.}\\
    Is bovenstaande bewering altijd waar, soms waar of nooit waar. Verklaar.
  \end{enumerate}
\end{oefening}
